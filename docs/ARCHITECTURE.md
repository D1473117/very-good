...
### 決策 5：位置定位防錯與「手動地標/區域」備用 (Fallback) 機制
- **做法**：在首頁探索定位卡片中，新增了一個精緻的毛玻璃手動地標下拉選單（`#select-landmark`），內建北中兩地核心地標商圈座標。使用者選取特定地標後，系統會自動在前端解析對應的經緯度，並標記為當前參考點。
- **原因**： HTML5 GPS 定位高度依賴裝置環境、瀏覽器隱私政策與使用者的存取授權，在實務上很容易因被拒絕或信號不佳而失敗。透過提供手動選單，完美實現了 PRD 規劃中「當定位拒絕時，允許手動輸入/選擇區域或地標」的防錯機制，確保使用者不論在什麼設備與環境下，都能享有精準的「附近餐廳」推薦服務。
